...
export default function Canicule() {
  ...
  const codes = Object.keys(cfg.d.tot);
  const value = c => year === 'all' ? cfg.d.tot[c] : (cfg.d.per[c][year] || 0);
  const years = []; for (let y = cfg.y0; y <= cfg.y1; y++) years.push(y);
  ...
  const classOf = v => { if (v <= 0) return -1; for (let i = 0; i < 5; i++) if (v < bk[i]) return i; return 5; };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 14 }}>
        <label style={{ color: 'var(--muted)', fontSize: 12 }}>Période</label>
      ...
      <div className="grid2">
        <div className="card" style={{ position: 'relative' }}>
          <h2>Carte — {year === 'all' ? `cumul ${cfg.y0}-${cfg.y1}` : year}</h2>
            ...
            {RAMP.map((col, i) => (
              <div key={i} style={{ flex: 1 }}>
                ...
                <div style={{ color: 'var(--muted)', fontSize: 11, marginTop: 4, fontVariantNumeric: 'tabular-nums' }}>
                  {i === 0 ? `< ${fmtv(bk[0])}` : i === 5 ? `≥ ${fmtv(bk[4])}` : `${fmtv(bk[i - 1])}–${fmtv(bk[i])}`}
                </div>
              </div>
            ))}
          </div>
          {tip && (() => {
            ...
            const mx = Math.max(...years.map(y => cfg.d.per[c][y] || 0), 1);
            return (
              <div style={{
        ...
        <div className="card">
          <h2>Classement — 20 départements les plus touchés</h2>
            ...
            {ranked.slice(0, 20).map((c, i) => {
              ...
              return (
                <div key={c}
                  ...
                  <div style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>{fmtv(v)}</div>
                </div>
              );
            })}
          </div>
      ...
      <div className="warnbox">
        ...
        autres onglets). Sources : Santé publique France (ODISSE), ODRÉ, fond de carte IGN/france-geojson.
      </div>
    </div>
  );
}
